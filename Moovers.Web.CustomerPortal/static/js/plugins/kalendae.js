...
(function(undefined) {
    ...
    Kalendae.Input = function(targetElement, options) {
        ...
        var setDate = function(input) {
            self.viewStartDate = Kalendae.moment(input.value);
            self.setSelected(input.value);
        }
        ...
        self.subscribe('change', function() {
            var fromCal = Kalendae.moment(self.getSelected());
            var cur = Kalendae.moment($input.value);

            if (!self.getSelected()) return;

            if (cur.diff(fromCal) !== 0) {
                $input.value = self.getSelected();
                if (jQuery) {
                    jQuery($input).trigger("change").trigger("select");
                }
            }
        });
    };
