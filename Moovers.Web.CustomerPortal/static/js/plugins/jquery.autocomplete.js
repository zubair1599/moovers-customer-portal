...
(function($) {
    ...
    Autocomplete.prototype = {
        ...
        enableKillerFn: function() {
            var me = this;
            $(document).bind('click touchstart', me.killerFn);
        },

        disableKillerFn: function() {
            var me = this;
            $(document).unbind('click touchstart', me.killerFn);
        },
        ...
        getSuggestionsLocal: function(q) {
            var ret, arr, len, val, i, j, option, options, isMatch, order, max;
            ...
            // find an item by name or "Alts"
            for (i = 0; i < len; i++) {
                val = arr.suggestions[i];
                if ($.isPlainObject(val) && val.value && val.alts) {
                    options = [val.value].concat(val.alts).map(function(i) {
                        return {
                            value: i,
                            partial: val.value ===  i || !parseFloat(i) // special case - if one of the "Alts" is numeric, only match on the entire string exactly
                        };
                    });
                ...
                for (j = 0; j < options.length; j++) {
                    ...
                    else if (this.options.partialMatch && option.partial && (option.value.toLowerCase().indexOf(q) >= 0)) {
                        isMatch = true;
                    }
                    else if (!this.options.partialMatch && option.partial && (option.value.toLowerCase().indexOf(q) === 0)) {
                        isMatch = true;
                    }
                    else if (!option.partial && option.value.toLowerCase() === q) {
                        isMatch = true;
                    }
            ...
            order = $.isFunction(this.options.orderBy) ? this.options.orderBy(q) : undefined;
            max = this.options.maxSuggestions;
            if (max > 0) {
                ret.suggestions = ret.suggestions.sort(order).slice(0, max);
            }
        ...
        suggest: function() {
            ...
            for (i = 0; i < len; i++) {
                ...
                div.bind("click touchstart", mClick(i));
